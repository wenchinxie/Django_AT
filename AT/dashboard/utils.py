...
class update_data:
    ...
    def data(self, _):
        ...
        df = pd.read_sql(
            f"SELECT Date,Material_name,Price FROM {self.data_type}",
            self.conn,
            parse_dates="Date",
        )
    ...
    def update_data_if_needed(self):
        ...
        try:
            ...
            cursor = self.conn.execute(
                text(f"SELECT * FROM {self.data_type} ORDER BY Date DESC LIMIT 1;")
            )
            row = cursor.fetchone()
        ...
        if recent_date.date() != lastest_bday.date():
            ...
            renew_data.to_sql(
                self.data_type, self.conn, if_exists="append", index=False
            )
    ...
    def table_calculation(self, df):
        ...
        def calc_percent_change(period):
            ...
            price_prev = data[
                data["Date"]
                == (data["Date"][0] - pd.DateOffset(**period_mapping[period]))
            ]["Price"].values[0]
